...
export async function fetchNBAStandings(
  season: string,
): Promise<NBATeamStanding[]> {
  const basketballBase =
    import.meta.env.VITE_BASKETBALL_API_BASE_URL ?? "/api-basketball";

  const url = basketballBase.includes(".netlify")
    ? `${basketballBase}?league=12&season=${season}`
    : `${basketballBase}/standings?league=12&season=${season}`;
